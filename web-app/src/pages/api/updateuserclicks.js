        ...
        try {
            ...
            if (campaignCollection && tokenCollection) {
                ...
                const campaignUpdate = await db.collection("campaigns").updateOne(
                    ...
                    {
                        $set: { clicks: campaignCollection.clicks + 1, balance: reducedAmount }
                    }
                )
                ...
                if (tokenCollection) {
                    const tokenUpdate = await db.collection("tokens").updateOne(
                        ...
                        {
                            $set: { clickCount: tokenCollection.clickCount + 1, balance: tokenCollection.balance + campaignCollection.payclick }
                        }
                    )
